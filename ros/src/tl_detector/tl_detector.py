...
from styx_msgs.msg import Light
...
class TLDetector(object):
    def __init__(self):
        ...
        rospy.logerr(config_string)
        
        self.config = yaml.load(config_string)
        rospy.logerr("Loaded yaml")
        rospy.logerr(self.config)
        
        #model_path = self.config['tl']['model']
        #model_path = self.config['tl']['tl_classification_model']
        model_path = self.config['classification_model']
        
        resize_width = self.config['camera_info']['image_width']
        resize_height = self.config['camera_info']['image_height']
    ...
    def image_cb(self, msg):
        ...
        '''
        if self.state != state:
            rospy.logerr("STATE ne STATE")
            self.state_count = 0
            self.state = state
        elif self.state_count >= STATE_COUNT_THRESHOLD:
            rospy.logerr("STATE COUNT ge STATE COUNT THRESH")
